...
# --------------------------------------------------------------------
# FWHM Calculation Function (Unchanged)
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# Simulation (Collects data for FOM calculation)
# --------------------------------------------------------------------
wavelengths = np.linspace(880, 1100, 1000000)
...
resonance_peaks = [] # Store peak wavelengths
fwhm_values = []     # Store FWHM values
...
# 2. Average FWHM values to align with sensitivity data points
fwhm_array = np.array(fwhm_values)
fwhm_midpoints = (fwhm_array[:-1] + fwhm_array[1:]) / 2.0

# 3. Calculate FOM = S / FWHM
# Use a where clause to avoid division by zero or NaN
fom = np.divide(sensitivity, fwhm_midpoints, out=np.full_like(sensitivity, np.nan), where=fwhm_midpoints>0)

# Also get concentration midpoints for plotting
concentration_midpoints = (atrazine_conc_uM[:-1] + atrazine_conc_uM[1:]) / 2.0


# ----------------------------------------------------------------
# --- CSV File Generation (Updated for FOM) ---
# ----------------------------------------------------------------
csv_filename = 'figure_of_merit_data.csv'
print(f"Generating {csv_filename}...")
with open(csv_filename, 'w', newline='') as csvfile:
    csv_writer = csv.writer(csvfile)
    # Write Header
    csv_writer.writerow(['No of period', 'Figure of Merit (uM^-1)', 'Concentration (uM)'])
    # Write Data
    for conc, f in zip(concentration_midpoints, fom):
        csv_writer.writerow([N, f, conc])
print("CSV file generated successfully.")


# --- PLOTTING (Updated for Figure of Merit) ---

# Plot FOM vs Concentration
plt.figure(figsize=(8, 6))
plt.plot(concentration_midpoints, fom, 'mo-', markersize=5, linewidth=2)
plt.xscale('log')
plt.xlabel("Atrazine Concentration (µM)", fontsize=14)
plt.ylabel("Figure of Merit (µM⁻¹)", fontsize=14)
plt.title(f"Sensor Figure of Merit at {ANGLE_DEG}° Incidence ({POLARIZATION.upper()}-pol)", fontsize=16)
plt.grid(True, which="both", linestyle='--', alpha=0.7)
plt.tight_layout()

# Plot Transmission Spectra (Unchanged, for context)
plt.figure(figsize=(8, 6))
