...
d_TiO2 = 800 / (4 * n_TiO2)
d_SiO2 = 800 / (4 * n_SiO2)
d_deviation = 0.01 # 5% deviation
d_MED = (800 / (2 * n_defect_initial)) * (1 + d_deviation)
d_defect = d_MED
...
# --- CSV File Generation ---
csv_filename = 'resonance_data.csv'
with open(csv_filename, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['Resonance Peak (nm)', 'Concentration (uM)', 'Defect Thickness (nm)'])
    for peak, conc in zip(resonance_peaks, atrazine_conc_uM):
        writer.writerow([peak, conc, d_defect])
...
plt.ylabel("Resonance Wavelength (nm)", fontsize=14)
