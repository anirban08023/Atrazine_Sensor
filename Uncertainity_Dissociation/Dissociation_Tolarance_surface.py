...
# --- Load Data ---
csv_filename = 'Dissociation_Tolerance.csv'
try:
...
z = df['Resonance Peak Shift (nm)']
x = df['Concentration (uM)']
y = df['Dissociation Constant (uM^-1)']
...
x_grid, y_grid = np.mgrid[log_x.min():log_x.max():200j, y.min():y.max():200j]

# Interpolate the scattered z data onto the uniform grid.
# The 'cubic' method creates a smooth surface.
z_grid = griddata((log_x, y), z, (x_grid, y_grid), method='cubic')
...
# --- Customize Plot ---
ax.set_xlabel('Log10(Concentration (µM))', fontsize=16, labelpad=15)
ax.set_ylabel('Dissociation Constant (µM^-1)', fontsize=16, labelpad=15)
ax.set_zlabel('Resonance Peak Shift (nm)', fontsize=16, labelpad=15)
ax.tick_params(axis='z', pad=10)


# Add a color bar which maps values to colors.
cbar = fig.colorbar(surf, shrink=0.6, aspect=10)
cbar.set_label('Resonance Peak Shift (nm)', fontsize=16)
