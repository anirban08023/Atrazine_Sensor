...
# --------------------------------------------------------------------
# Simulation and Plotting (Updated to calculate Quality Factor)
# --------------------------------------------------------------------
wavelengths = np.linspace(880, 980, 100000)
...
q_factor_values = [] # MODIFIED: Store Q-factor values
T_lowest_conc, T_highest_conc = None, None
print("Simulating transmission and calculating Quality Factor for each concentration...")
...
for i, n_defect_current in enumerate(n_defect_array):
    ...
    # Find the peak to calculate its properties
    ...
    # --- MODIFIED: Calculate Q-Factor ---
    if peak_index != -1:
        fwhm = calculate_fwhm(wavelengths, T, peak_index)
        # Q-factor is resonance_wavelength / FWHM
        if fwhm is not np.nan and fwhm > 0:
            q_factor = peak_wavelength / fwhm
        else:
            q_factor = np.nan # Can't calculate Q if FWHM is invalid
        q_factor_values.append(q_factor)
    else:
        q_factor_values.append(np.nan)

print("Simulation complete.")

# ----------------------------------------------------------------
# --- CSV File Generation (Updated for Quality Factor) ---
# ----------------------------------------------------------------
csv_filename = 'quality_factor_data.csv'
print(f"Generating {csv_filename}...")
with open(csv_filename, 'w', newline='') as csvfile:
    csv_writer = csv.writer(csvfile)
    # Write Header
    csv_writer.writerow(['n_mip_polymer', 'Concentration (uM)', 'Quality Factor'])
    # Write Data
    for conc, qf in zip(atrazine_conc_uM, q_factor_values):
        csv_writer.writerow([n_mip_polymer, conc, qf])
print("CSV file generated successfully.")


# --- PLOTTING (Updated for Quality Factor) ---

# Plot Quality Factor vs Concentration
plt.figure(figsize=(8, 6))
plt.plot(atrazine_conc_uM, q_factor_values, 'go-', markersize=5, linewidth=2)
plt.xscale('log')
plt.xlabel("Atrazine Concentration (µM)", fontsize=14)
plt.ylabel("Quality Factor (Q)", fontsize=14)
plt.title(f"Sensor Quality Factor at {ANGLE_DEG}° Incidence ({POLARIZATION.upper()}-pol)", fontsize=16)
